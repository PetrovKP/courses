...
static inline void Syrk(double* result, const double* const A, int n, const Uplo uplo) {
    for (int i = 0; i < n; i++) {
        for (int j = i; j < n; j++) {
            double sum = 0.0;
...
template <typename T>
static inline void GeneratePositiveDefiniteMatrix(T* A, const int n, RandomGenerator& random)
{
